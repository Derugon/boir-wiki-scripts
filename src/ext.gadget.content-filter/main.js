...
const messages = {
	base: 'gadget-content-filter-list',
	toggle: 'gadget-content-filter-list-toggle',
	title: 'gadget-content-filter-list-title',
	name: 'gadget-content-filter-list-name'
};
...
class Navigation {
	 ...
	 */
	constructor( title, buttonTitles, buttonNames ) {
...
const updateView = ( index ) => {
	...
	if ( index !== null ) {
		...
		for ( const viewFragment of queryElementsByClassName( `${cfView.css.viewClassPrefix}${index}` ) ) {
			viewFragment.classList.add( css.activeViewClass );
		}

		for ( const viewMarker of queryElementsByClassName( `${cfView.css.viewMarkerClassPrefix}${index}` ) ) {
			viewMarker.click();
		}
	}
...
if ( [ 'view', 'edit', 'submit' ].includes( config.wgAction ) ) {
	$( () => {

...
const addPageLink = () => {
	...
	firstHeadingTitle.replaceWith( anchor );
	anchor.append( firstHeadingTitle );
};

/**
 * Add an anchor on the page title text.
 *
 * @param {HTMLElement | null} element Element to check the content of.
 * @param {boolean} [acceptWhole] Whether an anchor can be added if the page title covers the whole node content.
 */
const addPageLinkToText = ( element, acceptWhole = false ) => {
	if ( element === null ) {
		return false;
	}
	...
	for ( const childNode of element.childNodes ) {
		if ( childNode instanceof Text ) {
			...
			element.textContent = '';
			element.append( parts[0], anchor, parts[1] );

			return true;
		} else if ( childNode instanceof HTMLElement ) {
			if ( !addPageLinkToText( childNode, acceptWhole ) ) {
				continue;
			}
